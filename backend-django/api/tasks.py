...
# Core upsert
# ──────────────────────────────────────────────
def upsert_anime(mal_id, title, url, current_category, personal_rating=None):
        ...
        try:
            ...
            for i, anime_elem in enumerate(entries):
                ...
                if mal_id_elem is not None and title_elem is not None and mal_id_elem.text:
                    ...
                    try:
                        _, created = upsert_anime(mal_id, title, url, category, personal_rating)
            ...
            _finish_import(added, updated, errors)
            return
    ...
    try:
        ...
        for category, items in data.items():
            for item in items:
                ...
                if mal_id and title:
                    try:
                        _, created = upsert_anime(mal_id, title, url, category)
                        safe_title = title.encode('ascii', 'replace').decode('ascii')
                        if created:
                            added += 1
...
def _finish_import(added, updated, errors):
    _log(f'')
